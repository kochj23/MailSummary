...
This app exposes a local HTTP API on port **37439** for integration with [Nova](https://github.com/kochj23) (OpenClaw AI) and Claude Code.
...
```bash
curl http://127.0.0.1:37439/api/status   # App status + uptime
curl http://127.0.0.1:37439/api/ping     # Health check
```
...
```bash
# Check if running
curl -s http://127.0.0.1:37439/api/status | python3 -m json.tool
