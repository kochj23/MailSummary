...
class NovaAPIServer {
    ...
    func start() {
        do {
            ...
            listener?.stateUpdateHandler = { if case .ready = $0 { print("NovaAPI [MailSummary]: port \(self.port)") } }
            listener?.start(queue: .main)
        } catch { print("NovaAPI [MailSummary]: failed — \(error)") }
    }
    ...
    private func route(_ req: NovaRequest) async -> String {
        ...
        case ("GET", "/api/status"):
            return json(200, ["status": "running", "app": "MailSummary", "version": "1.0", "port": "\(port)", "uptimeSeconds": Int(Date().timeIntervalSince(startTime))] as [String: Any])
        case ("GET", "/api/ping"):
